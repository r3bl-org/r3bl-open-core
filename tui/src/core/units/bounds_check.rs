...
/// if range.is_valid_for(buffer_length) { ... }
/// ```
///
/// ## Usage Examples
///
/// ```rust
/// use std::ops::Range;
/// use crate::{ColIndex, ColWidth, col, width};
///
/// let buffer_width = width(10);  // Buffer has 10 columns (0-9)
///
/// // Valid ranges:
/// let range1: Range<ColIndex> = col(0)..col(5);   // columns 0-4
/// let range2: Range<ColIndex> = col(5)..col(10);  // columns 5-9 (end=10 is valid!)
/// let range3: Range<ColIndex> = col(0)..col(10);  // entire buffer
///
...
///
/// This provides type-safe validation for ranges of any index type (ColIndex, RowIndex,
/// etc.) against their corresponding length types.
...
#[cfg(test)]
mod tests_bounds_overflow_status {
    use super::*;
...
#[cfg(test)]
mod tests_content_position_status {
    use super::*;
...
#[cfg(test)]
mod tests_content_position_checking {
    use super::*;
    use crate::{ColIndex, ColWidth, RowHeight, RowIndex, idx};
