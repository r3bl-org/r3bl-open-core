...
///
/// This struct is shared between [`TuiStyle`] (which adds [`id`], [`computed`],
/// [`padding`], [`lolcat`]) and [`AnsiToOfsBufPerformer`] (which uses these for [`ANSI`]
/// sequence processing).
///
/// ## Usage Patterns
///
...
/// [`padding`]: crate::TuiStyle::padding
...
///
/// ```rust
/// use r3bl_tui::{TuiStyleAttribs, tui_style_attribs, tui_style_attrib::{Bold, Italic, Dim}};
///
/// // 1. Single attribute.
/// let bold_style = TuiStyleAttribs::from(Bold);
/// let also_bold = tui_style_attribs(Bold);
///
/// // 2. Multiple attributes combined with `+`.
/// let bold_italic: TuiStyleAttribs = Bold + Italic;
///
/// // 3. Mutating existing attributes with `+=`.
/// let mut attribs = TuiStyleAttribs::default();
/// attribs += Dim;
/// ```
///
/// The pattern uses a newtype approach where each attribute is its own unit struct, and
/// [`TuiStyleAttribs`] stores them as [`Option<T>`]. This ensures type safety and clarity
/// while providing a very ergonomic way to compose complex styles.
