...
//! Extension trait for [`Range`] (aka [`RangeExclusive`]) and [`RangeInclusive`]
//! validation for iteration and algorithms - see [`RangeBoundsExt`] trait.
//!
//! [`Range`]: std::ops::Range

use crate::{ArrayBoundsCheck, ArrayOverflowResult, CursorBoundsCheck,
            CursorPositionBoundsStatus, IndexOps, LengthOps, NumericValue,
            RangeBoundsResult, RangeExclusive, RangeValidityStatus};
use std::ops::{Add, RangeInclusive};

/// Extension trait for [`RangeExclusive<Index>`] and [`RangeInclusive<Index>`] validation for
...
/// # Purpose
///
/// This trait answers the question: "Is this [`Range`] or [`RangeInclusive`] valid for
/// content of a given length?"
///
...
///
/// **Exclusive Range ([`RangeExclusive<Index>`]) Validation**
///
/// For [`RangeExclusive<Index>`], the end can equal content length because it's NOT included:
///
/// ```text
...
/// [`RangeExclusive<Index>`]: std::ops::Range
...
where
    Self::IndexType:
        Add<Output = Self::IndexType>            /* Allows adding 2 index types together */
        ...
        + Default,                               /* Allows constructing a zero index */
{
    ...
    type LengthType: CursorBoundsCheck + LengthOps<IndexType = Self::IndexType>;
...
/// Implementation of range operations for [`RangeExclusive<IndexType>`].
///
/// This provides type-safe validation and clamping for ranges of any index type
/// ([`VPCol`], [`VPRow`], etc.) against their corresponding length types.
///
...
///
/// [`RangeExclusive<IndexType>`]: std::ops::Range
/// [`VPCol`]: crate::VPCol
/// [`VPRow`]: crate::VPRow
#[rustfmt::skip]
impl<I> RangeBoundsExt for RangeExclusive<I>
where
    I:
        IndexOps                          /* Provides index operations and comparisons */
        + ArrayBoundsCheck<I::LengthType> /* Ensure index can check overflow against this length */
        + Add<Output = I>                 /* Allows adding 2 index types together */
        + Add<usize, Output = I>          /* Allows adding usize to an index type */
        + Default,                        /* Allows constructing a zero index */
{
    type IndexType = I;
    ...
    fn clamp_range_to(self, buffer_length: Self::LengthType) -> RangeExclusive<I> {
...
mod tests_range_validation {
    use super::*;
    use crate::{VPIndex, vp_idx, vp_len};

    #[test]
    fn test_range_validation_valid_ranges() {
        let content_width = vp_len(10); // Columns 0-9

        // Valid ranges
        let range1: RangeExclusive<VPIndex> = vp_idx(0u16)..vp_idx(5u16); // columns 0-4
        let range2: RangeExclusive<VPIndex> = vp_idx(5u16)..vp_idx(10u16); // columns 5-9 (end=10 is valid!)
        let range3: RangeExclusive<VPIndex> = vp_idx(0u16)..vp_idx(10u16); // entire content
        let range4: RangeExclusive<VPIndex> = vp_idx(7u16)..vp_idx(9u16); // middle range
        let empty_range: RangeExclusive<VPIndex> = vp_idx(5u16)..vp_idx(5u16); // empty range
    ...
    fn test_range_validation_invalid_ranges() {
        ...
        let bad_range1: RangeExclusive<VPIndex> = vp_idx(5u16)..vp_idx(11u16); // end > len
        let bad_range2: RangeExclusive<VPIndex> = vp_idx(0u16)..vp_idx(11u16); // end > len
        let bad_range3: RangeExclusive<VPIndex> = vp_idx(15u16)..vp_idx(20u16); // start beyond content
        ...
        let inverted_range: RangeExclusive<VPIndex> = vp_idx(8u16)..vp_idx(3u16); // inverted range
        assert_eq!(
...
mod tests_range_clamp {
    ...
    fn test_range_clamp_to_content_end_beyond_bounds() {
        ...
        let range1: RangeExclusive<VPIndex> = vp_idx(5u16)..vp_idx(15u16);
        let clamped1 = range1.clamp_range_to(content_width);
        assert_eq!(
            clamped1,
            vp_idx(5u16)..vp_idx(10u16),
        ...
        let range2: RangeExclusive<VPIndex> = vp_idx(0u16)..vp_idx(15u16);
        ...
        let range3: RangeExclusive<VPIndex> = vp_idx(20u16)..vp_idx(30u16);
        let clamped3 = range3.clamp_range_to(content_width);
        assert_eq!(
            clamped3,
            vp_idx(0u16)..vp_idx(0u16),
    ...
    fn test_range_clamp_exclusive_end_semantics() {
        ...
        let range_to_end: RangeExclusive<VPIndex> = vp_idx(5u16)..vp_idx(10u16);
        ...
        let last_element: RangeExclusive<VPIndex> = vp_idx(9u16)..vp_idx(10u16);
        let clamped_last = last_element.clamp_range_to(content_width);
        assert_eq!(
            clamped_last,
            vp_idx(9u16)..vp_idx(10u16),
    ...
    fn test_zero_length_content() {
        let zero_content = vp_len(0);

        // Any range with zero-length content should become empty at 0
        let range1: RangeExclusive<VPIndex> = vp_idx(0u16)..vp_idx(5u16);
        ...
        let range2: RangeExclusive<VPIndex> = vp_idx(5u16)..vp_idx(10u16);
...
mod tests_range_inclusive_validation {
    ...
    fn test_range_inclusive_validation_valid_ranges() {
        ...
        let range1: RangeInclusive<VPIndex> = vp_idx(0u16)..=vp_idx(4u16); // columns 0-4
        let range2: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(9u16); // columns 5-9
        let range3: RangeInclusive<VPIndex> = vp_idx(0u16)..=vp_idx(9u16); // entire content
        let range4: RangeInclusive<VPIndex> = vp_idx(7u16)..=vp_idx(9u16); // end range
        let single: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(5u16); // single element
    ...
    fn test_range_inclusive_validation_invalid_ranges() {
        ...
        let bad_range1: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(10u16); // end == len is INVALID
        let bad_range2: RangeInclusive<VPIndex> = vp_idx(0u16)..=vp_idx(10u16); // end == len is INVALID
        let bad_range3: RangeInclusive<VPIndex> = vp_idx(0u16)..=vp_idx(11u16); // end > len
        let bad_range4: RangeInclusive<VPIndex> = vp_idx(15u16)..=vp_idx(20u16); // start beyond content
        ...
        // Invalid ranges - inverted
        let inverted: RangeInclusive<VPIndex> = vp_idx(8u16)..=vp_idx(3u16);
        assert_eq!(
...
mod tests_range_inclusive_clamp {
    ...
    fn test_range_inclusive_clamp_normal_cases() {
        ...
        // Normal range within bounds - should remain unchanged
        let range1: RangeInclusive<VPIndex> = vp_idx(2u16)..=vp_idx(7u16);
        let clamped1 = range1.clamp_range_to(content_width);
        assert_eq!(
            clamped1,
            vp_idx(2u16)..=vp_idx(7u16),
        ...
        let full_range: RangeInclusive<VPIndex> = vp_idx(0u16)..=vp_idx(9u16);
        let clamped_full = full_range.clamp_range_to(content_width);
        assert_eq!(
            clamped_full,
            vp_idx(0u16)..=vp_idx(9u16),
            "Full content range should remain unchanged"
        );

        // Single-element range at end - should remain unchanged
        let last_element: RangeInclusive<VPIndex> = vp_idx(9u16)..=vp_idx(9u16);
        let clamped_last = last_element.clamp_range_to(content_width);
        assert_eq!(
            clamped_last,
            vp_idx(9u16)..=vp_idx(9u16),
    ...
    fn test_range_inclusive_clamp_end_beyond_bounds() {
        ...
        let range1: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(10u16);
        let clamped1 = range1.clamp_range_to(content_width);
        assert_eq!(
            clamped1,
            vp_idx(5u16)..=vp_idx(9u16),
            "Range 5..=10 should clamp end to 9 (last valid index)"
        );

        // Range 0..=10 should clamp to 0..=9
        let range2: RangeInclusive<VPIndex> = vp_idx(0u16)..=vp_idx(10u16);
        let clamped2 = range2.clamp_range_to(content_width);
        assert_eq!(
            clamped2,
            vp_idx(0u16)..=vp_idx(9u16),
        ...
        let range3: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(15u16);
        let clamped3 = range3.clamp_range_to(content_width);
        assert_eq!(
            clamped3,
            vp_idx(5u16)..=vp_idx(9u16),
        ...
        // Range with both start and end way beyond bounds
        let range4: RangeInclusive<VPIndex> = vp_idx(20u16)..=vp_idx(30u16);
    ...
    fn test_range_inclusive_inverted_range_clamping() {
        ...
        let inverted_boundary: RangeInclusive<VPIndex> = vp_idx(9u16)..=vp_idx(5u16);
        let clamped_boundary = inverted_boundary.clamp_range_to(content_width);
        assert_eq!(
            clamped_boundary,
            vp_idx(9u16)..=vp_idx(9u16),
    ...
    fn test_range_inclusive_zero_length_content() {
        ...
        let range1: RangeInclusive<VPIndex> = vp_idx(0u16)..=vp_idx(5u16);
        let clamped1 = range1.clamp_range_to(zero_content);
        assert_eq!(
            clamped1,
            vp_idx(0u16)..=vp_idx(0u16),
        ...
        let range2: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(10u16);
        let clamped2 = range2.clamp_range_to(zero_content);
        assert_eq!(
            clamped2,
            vp_idx(0u16)..=vp_idx(0u16),
...
mod tests_compare_exclusive_vs_inclusive {
    ...
    fn test_range_inclusive_semantic_difference() {
        ...
        let exclusive: RangeExclusive<VPIndex> = vp_idx(5u16)..vp_idx(10u16);
        assert_eq!(
        ...
        let inclusive: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(10u16);
        assert_eq!(
        ...
        let correct_inclusive: RangeInclusive<VPIndex> = vp_idx(5u16)..=vp_idx(9u16);
        assert_eq!(
...
mod tests_range_membership {
    use super::*;
    use crate::{vp_col, vp_idx, vp_row};

    #[test]
    fn test_check_index_is_within_exclusive() {
        let range = vp_col(3)..vp_col(8); // Exclusive: [3, 8)

        // Test underflowed
        assert_eq!(
            range.check_index_is_within(vp_col(0)),
            RangeBoundsResult::Underflowed,
            "Index 0 should underflow [3, 8)"
        );
        assert_eq!(
            range.check_index_is_within(vp_col(2)),
            RangeBoundsResult::Underflowed,
        ...
        // Test within bounds
        assert_eq!(
            range.check_index_is_within(vp_col(3)),
            RangeBoundsResult::Within,
            "Index 3 should be within [3, 8) (start included)"
        );
        assert_eq!(
            range.check_index_is_within(vp_col(5)),
            RangeBoundsResult::Within,
            "Index 5 should be within [3, 8)"
        );
        assert_eq!(
            range.check_index_is_within(vp_col(7)),
            RangeBoundsResult::Within,
        ...
        assert_eq!(
            range.check_index_is_within(vp_col(8)),
            RangeBoundsResult::Overflowed,
            "Index 8 should overflow [3, 8) (end excluded)"
        );
        assert_eq!(
            range.check_index_is_within(vp_col(10)),
            RangeBoundsResult::Overflowed,
    ...
    fn test_check_index_is_within_all_states() {
        ...
        let range = vp_idx(5u16)..vp_idx(10u16); // Exclusive: [5, 10)

        let underflow_result = range.check_index_is_within(vp_idx(3u16));
        let within_result = range.check_index_is_within(vp_idx(7u16));
        let overflow_result = range.check_index_is_within(vp_idx(12u16));
    ...
    fn test_check_index_is_within_edge_cases() {
        ...
        let single_element_range = vp_idx(5u16)..vp_idx(6u16);

        assert_eq!(
            single_element_range.check_index_is_within(vp_idx(4u16)),
            RangeBoundsResult::Underflowed,
            "Index 4 should underflow [5, 6)"
        );
        assert_eq!(
            single_element_range.check_index_is_within(vp_idx(5u16)),
            RangeBoundsResult::Within,
            "Index 5 should be within [5, 6)"
        );
        assert_eq!(
            single_element_range.check_index_is_within(vp_idx(6u16)),
            RangeBoundsResult::Overflowed,
            "Index 6 should overflow [5, 6)"
        );

        // Test zero-width range [5, 5) - empty range
        let empty_range = vp_idx(5u16)..vp_idx(5u16);
        assert_eq!(
            empty_range.check_index_is_within(vp_idx(4u16)),
            RangeBoundsResult::Underflowed,
            "Index 4 should underflow empty range [5, 5)"
        );
        assert_eq!(
            empty_range.check_index_is_within(vp_idx(5u16)),
            RangeBoundsResult::Overflowed,
    ...
    fn test_check_index_is_within_inclusive_basic() {
        ...
        assert_eq!(
            scroll_region.check_index_is_within(vp_row(1)),
            RangeBoundsResult::Underflowed,
            "Row 1 should be before scroll region [2, 5]"
        );
        assert_eq!(
            scroll_region.check_index_is_within(vp_row(2)),
            RangeBoundsResult::Within,
            "Row 2 should be within scroll region [2, 5]"
        );
        assert_eq!(
            scroll_region.check_index_is_within(vp_row(3)),
            RangeBoundsResult::Within,
            "Row 3 should be within scroll region [2, 5]"
        );
        assert_eq!(
            scroll_region.check_index_is_within(vp_row(5)),
            RangeBoundsResult::Within,
            "Row 5 should be within scroll region [2, 5]"
        );
        assert_eq!(
            scroll_region.check_index_is_within(vp_row(6)),
            RangeBoundsResult::Overflowed,
    ...
    fn test_check_index_is_within_inclusive_edge_cases() {
        ...
        let single_element_range = vp_idx(5u16)..=vp_idx(5u16);
        assert_eq!(
            single_element_range.check_index_is_within(vp_idx(4u16)),
            RangeBoundsResult::Underflowed,
        ...
        let range = vp_idx(10u16)..=vp_idx(20u16);
        assert_eq!(
            range.check_index_is_within(vp_idx(9u16)),
            RangeBoundsResult::Underflowed,
            "Index 9 should underflow [10, 20]"
        );
        assert_eq!(
            range.check_index_is_within(vp_idx(10u16)),
            RangeBoundsResult::Within,
            "Index 10 should be within [10, 20]"
        );
        assert_eq!(
            range.check_index_is_within(vp_idx(20u16)),
            RangeBoundsResult::Within,
            "Index 20 should be within [10, 20]"
        );
        assert_eq!(
            range.check_index_is_within(vp_idx(21u16)),
            RangeBoundsResult::Overflowed,
