...
//! # Purpose
//!
//! These builders are **test-only convenience functions** (guarded by `#[cfg(any(test, doc))]`)
//! that wrap the underlying production sequence generation capabilities. While the sequence
//! types themselves are production code with bidirectional support (parsing + generation via
//! [`Display`]/[`FastStringify`]), these helpers provide simpler APIs for test code.
//!
...
//!
//! ```rust,ignore
//! use crate::vt_100_ansi_conformance_tests::test_sequence_builders::*;
//!
