...
pub fn delete_chars(processor: &mut AnsiToBufferProcessor, params: &vte::Params) {
    let chars_to_delete = MovementCount::from(params).as_usize();
    ...
    if current_col.as_usize() >= max_col.as_usize() {
        return;
    }

    // Calculate how many characters we can actually delete
    let actual_chars_to_delete = min(chars_to_delete, max_col.as_usize() - current_col.as_usize());

    // Shift characters left to fill the gap using copy_within
    let source_range = current_col.as_usize() + actual_chars_to_delete..max_col.as_usize();
    processor.ofs_buf.buffer[current_row.as_usize()].copy_within(source_range, current_col.as_usize());
...
pub fn insert_chars(processor: &mut AnsiToBufferProcessor, params: &vte::Params) {
    let chars_to_insert = MovementCount::from(params).as_usize();
    let current_row = processor.ofs_buf.my_pos.row_index;
    let current_col = processor.ofs_buf.my_pos.col_index;
    let max_col = processor.ofs_buf.window_size.col_width;
    ...
    let actual_chars_to_insert = min(chars_to_insert, max_col.as_usize() - current_col.as_usize());

    // Shift characters right using copy_within
    let destination_col = current_col.as_usize() + actual_chars_to_insert;
    let source_range = current_col.as_usize()..max_col.as_usize() - actual_chars_to_insert;
    processor.ofs_buf.buffer[current_row.as_usize()].copy_within(source_range, destination_col);

    // Insert blank characters at cursor position
    let insert_range = current_col.as_usize()..destination_col;
    processor.ofs_buf.buffer[current_row.as_usize()][insert_range].fill(PixelChar::Spacer);
}
...
pub fn erase_chars(processor: &mut AnsiToBufferProcessor, params: &vte::Params) {
    let chars_to_erase = MovementCount::from(params).as_usize();
    let current_row = processor.ofs_buf.my_pos.row_index;
    let current_col = processor.ofs_buf.my_pos.col_index;
    let max_col = processor.ofs_buf.window_size.col_width;
    ...
    let actual_chars_to_erase = min(chars_to_erase, max_col.as_usize() - current_col.as_usize());

    // Replace characters with blanks (no shifting)
    let erase_range = current_col.as_usize()..current_col.as_usize() + actual_chars_to_erase;
