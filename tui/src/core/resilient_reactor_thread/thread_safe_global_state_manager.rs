...
/// semantics.
///
/// # Why `Mutex<Option<Arc<ThreadState>>>`?
///
...
/// binary. [`Syscalls`] ask the OS to do something (create an [`epoll`] [`fd`], allocate
/// memory), which is impossible during compilation. The OS doesn't exist at compile time,
/// and these operations have side effects that can't be "undone."
///
...
/// runtime.
///
/// **Replacement on restart** — when the thread terminates and restarts (slow path), we
/// need to replace the entire [`ThreadState`] with fresh resources.
/// [`Option::replace()`] makes this clean.
///
/// **Note:** Fallibility is NOT the reason — we panic on [`syscall`] failure anyway. Even
/// if these operations were infallible, we'd still need [`Option<T>`] because they're not
/// `const`.
///
