...
//! - [Blocks on I/O] ([`stdin`], network, [signals])
//! - Broadcasts events to async consumers via [`broadcast`] channel
//! - Handles graceful shutdown when all consumers disconnect
//! - Supports thread restart/reuse with generation tracking
//!
...
//! [`kqueue`] (macOS).
//!
//! With these backends, the thread blocks in a [system call] (like [`poll()`] or
//! [`epoll_wait()`]) on one or more [file descriptors], waiting for **readiness** — a
...
//! | [`epoll`]/[`mio`] + [sockets]                | Yes     | Same blocking model                    |
//! | [`epoll`]/[`mio`] + [signals]                | Yes     | Via [`signalfd(2)`] or [`signal-hook`] |
//! | [`epoll`]/[`mio`] + [`pipe(2)`]/[`fifo(7)`]  | Yes     | Any [pollable] file descriptor         |
//!
//! The blocking behavior is identical regardless of **what** you're waiting on — stdin,
//! sockets, signals, or any pollable file descriptor. The thread blocks in
//! [`epoll_wait()`] until the kernel signals readiness.
//!
//! ## `io_uring` Compatibility
//!
//! For [`io_uring`], we recommend using [`io_uring_enter()`] with the wait flag. This
//! preserves the blocking behavior that RRT depends on, while gaining [`io_uring`]'s
//! performance benefits. The non-blocking modes ([`SQPOLL`], non-blocking completion
//! checks) break the RRT assumption and require a different pattern.
//!
//! See [`io_uring`: An Alternative Model] for detailed comparison, benefits, and
...
//! # Core Invariants
//!
//! 1. **Thread-safe global state lifecycle** — Static global with lazy initialization,
//!    thread-safe access via [`Mutex<Option<Arc<ThreadState>>>`]
//!
//! 2. **State machine** — Created/destroyed/reused with generation tracking to detect
//!    thread restarts vs thread reuse
//!
//! 3. **Contract preservation** — Async consumers never see broken promises; the
//!    [`broadcast`] channel decouples producers from consumers
//!
