...
/// for "jumbo" emoji like `🙏🏽`, which is represented by a single [`super::Seg`].
///
...
///
/// For performance reasons, the `GCStringOwned` struct owns the underlying string data.
...
///
/// We tried making a variant of `GCStringOwned` that does not own any data (the
...
/// local to `GCStringOwned`) via a different struct. This was unintuitive, as we were
...
/// use case it does not work for the `GCStringOwned` struct.
///
/// # Iterators
///
...
///
/// 1. [`Self::iter`]: Returns an iterator over the grapheme segments in the
///    `GCStringOwned`. This iterator returns the `&str` segments in the order they appear
///    in the underlying string. This makes it easy to iterate over the segments as `&str`
///    without knowing about the [Seg] struct.
/// 2. [`Self::seg_iter`]: Returns an iterator over the grapheme segments in the
///    `GCStringOwned`. This iterator returns the [Seg] segments in the order they appear
///    in the underlying string. This makes it easy to iterate over the segments as [Seg]
///    and all the low level details on byte offsets, display width, etc.
///
/// # Features
///
/// - `GCStringOwned`: Struct for representing Unicode strings with grapheme cluster
///   segmentation.
/// - [`Self::new`]: A constructor function for creating a `GCStringOwned` from a string
///   slice.
...
/// # Traits
///
/// The `GCStringOwned` struct implements the following traits:
/// - `Deref`: For dereferencing `GCStringOwned` instances. When the `*` operator is used,
///   the underlying `SegmentArray` is returned. This is really important to note when
///   using `len()`, which will return the number of grapheme clusters and this is not the
///   same as the `display_width` of the `GCStringOwned`.
/// - `DerefMut`: For mutable dereferencing of `GCStringOwned` instances.
/// - `Default`: For creating a default `GCStringOwned` instance.
/// - `PartialEq`: For comparing two `GCStringOwned` instances for equality.
/// - `Eq`: For checking if two `GCStringOwned` instances are equal.
/// - `Hash`: For hashing `GCStringOwned` instances.
/// - `Clone`: For creating a copy of a `GCStringOwned` instance.
/// - `Debug`: For debugging `GCStringOwned` instances.
/// - `SizeOf`: For calculating the size of `GCStringOwned` instances.
///
/// # Dependencies
///
...
///
/// ```
/// use r3bl_tui::graphemes::GCStringOwned;
///
/// let ustr = GCStringOwned::new("Hello, 世界");
...
/// list:
/// - [`GCStringOwned`] + [`ByteIndex`] = [Option]<[`SegIndex`]>
