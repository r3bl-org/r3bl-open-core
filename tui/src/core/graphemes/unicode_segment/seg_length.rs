...
use super::seg_index::SegIndex;
use crate::{ChUnit, LengthOps, generate_length_type_impl};
...
pub struct SegLength(ChUnit);
generate_length_type_impl!(SegLength, SegIndex, seg_length, seg_index);

impl SegLength {
    /// Converts the length to a segment index, by subtracting 1.
    #[must_use]
    pub fn convert_to_seg_index(&self) -> SegIndex { LengthOps::convert_to_index(self) }
}

impl From<SegIndex> for SegLength {
    fn from(other: SegIndex) -> SegLength { other.convert_to_seg_length() }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::{NarrowingCastToU16, seg_index};

    #[test]
    fn seg_length_conversions() {
        let length = seg_length(1u16);
    ...
    fn seg_length_from_various_types() {
        // Test From<usize>
        let length = SegLength::from(5u16);
        ...
        // Test From<i32>
        let length = SegLength::from(7i32.as_u16_narrowing());
