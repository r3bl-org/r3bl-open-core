...
/// Modifier parameter base character `'1'` (0x31 hex).
...
/// Control character range maximum (0x1F hex).
pub const CTRL_CHAR_RANGE_MAX: u8 = 31;

/// NUL (0x00 hex). Ctrl+Space or Ctrl+@.
pub const CONTROL_NUL: u8 = 0;

/// Tab (0x09 hex). Ctrl+I or Tab key.
pub const CONTROL_TAB: u8 = b'\t';

/// Line Feed (0x0A hex). Ctrl+J or Enter (Unix).
pub const CONTROL_LF: u8 = b'\n';

/// Carriage Return (0x0D hex). Ctrl+M or Enter (Windows/Mac).
pub const CONTROL_ENTER: u8 = b'\r';

/// Escape (0x1B hex). Ctrl+\[ or Esc key.
pub const CONTROL_ESC: u8 = 27;

/// Backspace (0x08 hex). Ctrl+H or Backspace key.
pub const CONTROL_BACKSPACE: u8 = 8;

/// Ctrl+C / ETX (0x03 hex). SIGINT in cooked mode.
pub const CONTROL_C: u8 = 3;

/// Ctrl+D / EOT (0x04 hex). EOF in cooked mode.
pub const CONTROL_D: u8 = 4;

/// Mask to convert control byte to lowercase (0x60 hex).
pub const CTRL_TO_LOWERCASE_MASK: u8 = 0b0110_0000;

/// Mask to convert control byte to uppercase (0x40 hex).
pub const CTRL_TO_UPPERCASE_MASK: u8 = 0b0100_0000;

/// Printable ASCII minimum: space (0x20 hex).
pub const PRINTABLE_ASCII_MIN: u8 = b' ';

/// Printable ASCII maximum: tilde (0x7E hex).
pub const PRINTABLE_ASCII_MAX: u8 = b'~';

/// DEL character (0x7F hex). Backspace key or Alt+Backspace with ESC.
...
/// `CSI I`: Focus gained (73, 0x49 hex).
pub const FOCUS_GAINED_FINAL: u8 = b'I';

/// `CSI O`: Focus lost (79, 0x4F hex).
...
/// Resize event code `'8'` for generation (56, 0x38 hex).
pub const RESIZE_EVENT_GENERATE_CODE: u8 = b'8';

/// Resize event terminator `'t'` (116, 0x74 hex).
