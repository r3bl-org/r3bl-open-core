...
//!
//! This module contains **automated** [`PTY`]-based integration tests that verify raw
//! mode behavior without requiring user interaction. All tests in this module can run in
//! CI environments.
//!
//! # Test Coverage
//!
...
//! # Manual Tests
//!
//! For tests that require a **real terminal** (e.g., `/dev/tty` fallback with redirected
//! stdin), see [`validation_tests`]. Those tests are marked `#[ignore]` and must be run
...
//!
//! ```bash
//! cargo test --package r3bl_tui terminal_raw_mode::raw_mode_integration_tests -- --nocapture
//! ```
//!
//! Run a specific test:
//!
//! ```bash
//! cargo test --package r3bl_tui test_raw_mode_pty -- --nocapture
//! ```
//!
//! # Architecture
//!
...
//! - Automatic cleanup
//!
//! [`PTY`] pairs simulate real terminals, allowing automated verification without user
//! interaction. For edge cases that cannot be simulated with PTYs (like stdin redirection
//! with controlling terminals), see [`validation_tests`].
//!
