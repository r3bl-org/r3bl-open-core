...
pub fn parse_keyboard_sequence(buffer: &[u8]) -> Option<(VT100InputEventIR, ByteOffset)> {
    if let Some(csi_u_event) = parse_csi_u_sequence(buffer) {
...
mod tests {
    ...
    fn test_parse_csi_u_alt_bracket() {
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input).expect("Should parse CSI u Alt+[");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
                code: VT100KeyCodeIR::Char('['),
    ...
    fn test_parse_csi_u_shift_enter() {
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input).expect("Should parse CSI u Shift+Enter");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
                code: VT100KeyCodeIR::Enter,
    ...
    fn test_parse_csi_u_ctrl_tab() {
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input).expect("Should parse CSI u Ctrl+Tab");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
                code: VT100KeyCodeIR::Tab,
    ...
    fn test_parse_csi_u_alt_escape() {
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input).expect("Should parse CSI u Alt+Escape");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
    ...
    fn test_parse_csi_u_event_types() {
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input_press).expect("Should parse press event");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
                code: VT100KeyCodeIR::Char('['),
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input_repeat).expect("Should parse repeat event");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
                code: VT100KeyCodeIR::Char('['),
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input_release).expect("Should parse release event");
    ...
    fn test_parse_csi_u_omitted_modifiers() {
        let input = b"\x1b[91u";
        let (event, consumed) =
            parse_keyboard_sequence(input).expect("Should parse plain CSI u");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
                code: VT100KeyCodeIR::Char('['),
    ...
    fn test_parse_csi_u_pua_functional_keys() {
        ...
        let (event, consumed) =
            parse_keyboard_sequence(input).expect("Should parse Shift+Home PUA");
        assert_eq!(
            event,
            VT100InputEventIR::Keyboard {
                code: VT100KeyCodeIR::Home,
