...
use crate::{ControlledChild, PtyPair,
            direct_to_ansi::{DirectToAnsiInputDevice,
                             input::{channel_types::StdinReaderMessage,
                                     global_input_resource::{guarded_ops, ThreadLiveness}}},
            generate_pty_test};
...
fn subscribe_controlled_entry_point() -> ! {
    ...
    runtime.block_on(async {
        ...
        // Read from device.
        let event_device =
            tokio::time::timeout(Duration::from_secs(5), device.try_read_event())
                .await
        ...
        let StdinReaderMessage::Event(event) = msg else {
