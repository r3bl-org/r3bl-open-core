...
//! ### 3. [`ESC`] + Other Byte
//!
//! When the buffer begins with [`ESC`] + (a byte other than `[` or `O`):
...
//!
//! ### 4. Non-[`ESC`] Sequences (Regular Input)
//!
...
/// accumulated sequence bytes and routes to specialized parsers ([`keyboard`],
/// [`mouse`], [`terminal_events`], [`utf8`]) based on content analysis.
///
/// # Arguments
///
...
///   - When `maybe_more.is_more_anticipated()` and `accumulated_bytes` is `[ESC]`: Return
///     `None` (wait for subsequent bytes of the multi-byte sequence).
///   - When `accumulated_bytes` is `[ESC]` and input is drained: Emit [`ESC`] key
///     immediately (0ms latency).
...
///    │
///    │ StatefulInputParser::advance(read_buffer, maybe_more)
///    ▼
/// ┌──────────────────────────────────────────┐  ┌──────────────────┐
...
/// as Up Arrow `\x1b[A` or OSC color queries `\x1b]11;...`) begin with the identical
/// byte [`ANSI_ESC`] (`0x1B`). When `accumulated_bytes` contains only `[0x1B]`, the
/// router must decide: is this a standalone [`ESC`] key, or the prefix of an in-flight
/// escape sequence?
///
/// ## The [`MaybeMore`] Stream Availability Heuristic
///
/// Instead of using a fixed 50-150ms timeout (which introduces sluggish latency for
/// modal editors like Vim), the parser inspects stream availability through
/// [`MaybeMore`]. See the [`MaybeMore`] documentation for the full two-level heuristic
/// model and pipeline architecture.
///
...
///    - `maybe_more.is_more_anticipated()` evaluates to `true`
///      ([`MaybeMore::RemainingInReadBuffer`] or [`MaybeMore::KernelMayHaveMore`]).
///    - The router returns `None` so the parser loop continues accumulating the rest of
///      the sequence.
///
/// 2. **Standalone [`ESC`] Key (Human Keystroke)**: A human pressing the physical [`ESC`]
///    key generates a single `0x1B` byte.
///    - `maybe_more.is_more_anticipated()` evaluates to `false` ([`MaybeMore::Drained`]).
///    - The router immediately returns `Some(VT100KeyCodeIR::Escape)` with **0ms
///      latency**.
///
...
/// | :------------------- | :------------------------- | :------------------------------------ |
/// | `[ 0x1B ]` alone     | `Drained`                  | Emit [`ESC`] key immediately (0ms)    |
/// | `[ 0x1B ]` alone     | `is_more_anticipated()`    | Return `None` (wait for more bytes)   |
...
/// │  │  ├─ maybe_more.is_more_anticipated()?               │
...
///   or if `maybe_more.is_more_anticipated()` is `true` and `accumulated_bytes` is
...
/// [`ESC`]: crate::EscSequence
/// [`keyboard`]: mod@super::keyboard
/// [`MaybeMore::Drained`]: super::MaybeMore::Drained
/// [`MaybeMore::KernelMayHaveMore`]: super::MaybeMore::KernelMayHaveMore
/// [`MaybeMore::RemainingInReadBuffer`]: super::MaybeMore::RemainingInReadBuffer
...
pub fn try_parse_input_event(
    ...
    match accumulated_bytes {
        ...
        // - maybe_more.is_more_anticipated(): Wait for more bytes (might be escape
        //   sequence).
        // - Otherwise: Emit ESC key immediately (no more input).
        [ANSI_ESC] if maybe_more.is_more_anticipated() => None,
...
mod tests_csi_routing {
    ...
    fn keyboard_arrow_key() {
        ...
        let (event, consumed) = try_parse_input_event(&buffer, MaybeMore::Drained)
    ...
    fn ss3_function_key() {
        ...
        let (event, consumed) =
            try_parse_input_event(buffer, MaybeMore::Drained).expect("Should parse F1");
...
mod tests_non_csi_input {
    ...
    fn utf8_char() {
        ...
        let buffer = generate_keyboard_sequence(&expected).expect("conversion error");
        let (event, consumed) =
            try_parse_input_event(&buffer, MaybeMore::Drained).expect("Should parse 'H'");
...
mod tests_invalid_input {
    ...
    fn empty_buffer_returns_none() {
        let buffer: &[u8] = &[];
        assert!(try_parse_input_event(buffer, MaybeMore::Drained).is_none());
    }
