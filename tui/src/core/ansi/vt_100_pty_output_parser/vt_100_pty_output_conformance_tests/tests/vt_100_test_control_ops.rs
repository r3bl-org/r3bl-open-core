...
pub mod basic_tab_operations {
    ...
    fn test_tab_with_text() {
        ...
        if let PixelChar::PlainText { display_char, .. } = &first_row[2] {
            assert_eq!(*display_char, 'C');
        }
        if let PixelChar::PlainText { display_char, .. } = &first_row[8] {
            assert_eq!(*display_char, 'D');
        }
        if let PixelChar::PlainText { display_char, .. } = &first_row[10] {
    ...
    fn test_tab_behavior_different_starting_positions() {
        ...
        for (start_col, expected_col) in test_positions {
            ...
            assert_eq!(
                ...
                start_col,
                expected_col,
                ofs_buf_vt_100.get_cursor_pos().col_index.as_usize()
            );
        }
    }
}
...
pub mod tab_edge_cases {
    ...
    fn test_tab_sequence_with_other_control_characters() {
        ...
        if let PixelChar::PlainText { display_char, .. } = &second_row[8] {
