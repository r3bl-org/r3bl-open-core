...
use crate::{CSI_START, ED_ERASE_DISPLAY, EL_ERASE_LINE, EraseDisplayMode, EraseLineMode,
            OfsBufVT100, PixelChar, PixelCharLine, TuiStyle,
...
fn create_filled_5r_by_5c_buffer() -> OfsBufVT100 {
    let mut buf = OfsBufVT100::new_empty(vp_height(5) + vp_width(5));
...
mod test_parameter_defaults {
    ...
    fn csi_j_defaults_to_mode_0() {
        ...
        ofs_buf_vt_100.set_cursor_pos(vp_row(2) + vp_col(2));
...
mod test_erase_display {
    ...
    fn mode_0_from_cursor_to_end() {
        let mut ofs_buf_vt_100 = create_filled_5r_by_5c_buffer();
        ofs_buf_vt_100.set_cursor_pos(vp_row(1) + vp_col(3));
    ...
    fn mode_3_entire_screen_and_scrollback() {
        ...
        ofs_buf_vt_100
            .primary_buffer_mut()
            .get_lines_mut()
            .push_front(PixelCharLine::new_empty(vp_width(0), PixelChar::Spacer));
        // Update viewport_row_offset so the viewport sees the same content
        ofs_buf_vt_100
            .primary_buffer_mut()
            .get_viewport_mut()
            .increment_history_len();
        assert_eq!(ofs_buf_vt_100.primary_buffer_mut().get_lines().len(), 6);
        ...
        // Scrollback should be cleared, leaving only viewport
        assert_eq!(ofs_buf_vt_100.primary_buffer_mut().get_lines().len(), 5);
    }
}
...
mod test_erase_line {
    ...
    fn mode_0_from_cursor_to_end() {
        let mut ofs_buf_vt_100 = create_filled_5r_by_5c_buffer();
        ofs_buf_vt_100.set_cursor_pos(vp_row(2) + vp_col(1));
