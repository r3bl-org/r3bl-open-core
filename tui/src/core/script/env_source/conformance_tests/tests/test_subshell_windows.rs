...
fn test_subshell_windows_cargo_env() -> miette::Result<()> {
    ...
    let formatted =
        run_fixture_bat(INPUT_CARGO_ENV_BAT, OutputFormat::Powershell, base_env)?;
...
fn test_subshell_windows_noisy_script() -> miette::Result<()> {
    ...
    let base_env = BaseEnv::Hermetic(mock_env);
    let formatted =
        run_fixture_bat(INPUT_NOISY_SCRIPT_BAT, OutputFormat::Powershell, base_env)?;
...
fn test_subshell_windows_edge_cases() -> miette::Result<()> {
    ...
    let json_out =
        run_fixture_bat(INPUT_EDGE_CASES_BAT, OutputFormat::Json, base_env.clone())?;
