...
use crate::{ArrayBoundsCheck, ArrayOverflowResult, CursorBoundsCheck, EditorArgsMut, ch,
            core::units::bounds_check::{IndexMarker, ViewportBoundsCheck}};
...
fn validate_vertical_scroll(args: EditorArgsMut<'_>) {
    ...
    // Check whether caret is within viewport.
    {
        ...
        let location = {
            let is_within =
                caret_scr_adj_row_index.is_in_viewport(scr_ofs_row_index, vp_height);
...
fn validate_horizontal_scroll(args: EditorArgsMut<'_>) {
    ...
    let is_within =
        if caret_scr_adj_col_index.is_in_viewport(scr_ofs_col_index, viewport_width) {
