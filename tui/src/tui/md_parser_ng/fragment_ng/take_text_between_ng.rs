...
/// contains a new line.
pub fn take_text_between_delims_enclosed_err_on_new_line_ng<'i>(
    input: AsStrSlice<'i>,
    start_delim: &'i str,
    end_delim: &'i str,
    ...
    match take_text_between_ng(start_delim, end_delim, input) {
        Ok((remaining, output)) => {
...
mod tests_parse_take_between {
    ...
    fn test_fenced_success() {
        ...
        let it = take_text_between_ng("_", "_", input);
        println!("it: {it:#?}");
    ...
    fn test_fenced_missing_end_tag() {
        ...
        let res = take_text_between_ng("_", "_", input);
        println!("it: {res:?}");
    ...
    fn test_parse_fenced_with_new_line_error() {
        ...
        let input = AsStrSlice::from(&lines);
        let it = take_text_between_delims_enclosed_err_on_new_line_ng(input, "_", "_");
        println!("it: {it:?}");
