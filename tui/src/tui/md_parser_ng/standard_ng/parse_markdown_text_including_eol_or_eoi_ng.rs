...
/// ([`crate::parse_line_empty_advance_ng`]) in the parser chain because
...
/// [`crate::parse_markdown_ng()`], typically as one of the last
...
mod inner {
    ...
    pub fn without_new_line<'a>(input: AsStrSlice<'a>) -> IResult<AsStrSlice<'a>, MdLineFragments<'a>> {
        ...
        let (input, output) = many0(
            |it| parse_inline_fragments_until_eol_or_eoi_ng(it, CheckboxParsePolicy::IgnoreCheckbox)
        ).parse(input)?;
