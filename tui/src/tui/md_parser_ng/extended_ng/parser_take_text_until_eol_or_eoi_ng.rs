...
#[rustfmt::skip]
pub fn parser_take_line_text_ng<'a>() ->
...
mod test_text_until_opt_eol {
    ...
    fn test_input_starts_with_new_line() {
        ...
        let (remainder, result) = parser_take_line_text_ng().parse(input).unwrap();
    ...
    fn test_input_with_eol() {
        ...
        let (rem, output) = parser_take_line_text_ng().parse(input).unwrap();
