...
///
/// The `split_by_new_line_ng` function performs a single character-iterator pass to identify
...
/// Look at the similar tests in the `tests_parse_block_code_ng_lines` module.
#[cfg(test)]
mod tests_parse_block_code_ng_single_line {
    use super::*;
    ...
    fn test_parse_codeblock_trailing_extra() {
        ...
        let (remainder, code_block_lines) = parse_block_code_advance_ng(input).unwrap();
        assert_eq2!(remainder.extract_to_slice_end().as_ref(), "");
        assert_eq2!(
            code_block_lines,
            convert_into_code_block_lines_ng(Some(lang_slice), &code_lines)
        );
    }
    ...
    fn test_parse_codeblock_no_language() {
        ...
        assert_eq2!(
            code_block_lines,
            convert_into_code_block_lines_ng(None, &code_lines)
        );
    }
}
...
#[cfg(test)]
mod tests_parse_code_block_lang_including_eol_ng {
    use super::*;
    use crate::{as_str_slice_test_case, assert_eq2};
    ...
    fn test_parse_code_block_lang_with_language() {
        // Test with language specified
        {
            ...
            let result = parse_code_block_lang_including_eol_ng(input);
...
#[cfg(test)]
mod tests_parse_code_block_body_including_code_block_end_ng {
    use super::*;
    use crate::{as_str_slice_test_case, assert_eq2};
    ...
    fn test_parse_code_block_body_simple_case() {
        // Test basic case with code content and closing tag
        {
            ...
            let result = parse_code_block_body_including_code_block_end_ng(input);
...
mod tests_convert_into_code_block_lines_ng {
    ...
    fn test_convert_into_code_block_lines_ng_with_language() {
        // Test with language and content lines
        {
            ...
            let result = convert_into_code_block_lines_ng(Some(lang), &lines);
    ...
    #[test]
    fn test_convert_into_code_block_lines_ng_without_language() {
        // Test without language
        {
            ...
            let result = convert_into_code_block_lines_ng(None, &lines);
    ...
    #[test]
    fn test_convert_into_code_block_lines_ng_empty_content() {
    ...
    #[test]
    fn test_convert_into_code_block_lines_ng_empty_lines() {
    ...
    #[test]
    fn test_convert_into_code_block_lines_ng_single_line() {
...
#[cfg(test)]
mod tests_split_by_new_line_ng {
    use super::*;
    use crate::{as_str_slice_test_case, assert_eq2};
    ...
    #[test]
    fn test_parse_codeblock_split_by_eol_ng() {
        // Test "foobar\n" -> ["foobar"]
        {
            as_str_slice_test_case!(input, "foobar\n");
            let result = split_by_new_line_ng(input);
...
mod tests_compat_with_original_split_by_new_line {
    ...
    fn test_parity_empty_string() {
        ...
        as_str_slice_test_case!(input, "");
        let alt_result = split_by_new_line_ng(input);
    ...
    #[test]
    fn test_ng_function_preserves_as_str_slice_properties() {
        // Test that the returned AsStrSlice instances maintain correct properties
        as_str_slice_test_case!(input, "foo\nbar\nbaz\n");
        let input_clone = input.clone();
        let results = split_by_new_line_ng(input);
    ...
    fn test_comprehensive_parity_test() {
        ...
        for test_input in test_cases {
            ...
            let alt_result = split_by_new_line_ng(input_slice);
            let alt_result_strings = slice_results_to_strings(&alt_result);
...
mod debug_tests {
    ...
    fn debug_test_parse_codeblock_multiple_lines() {
        ...
        let (remainder_after_body, body) =
            parse_code_block_body_including_code_block_end_ng(remainder_after_lang)
                .unwrap();
        ...
        // Test the splitting
        let split_result = split_by_new_line_ng(body);
