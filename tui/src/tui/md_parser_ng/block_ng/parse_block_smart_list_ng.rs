...
pub fn parse_block_smart_list_advance_ng<'a>(
    ...
    for (index, line) in smart_list_ir.content_lines.iter().enumerate() {
        ...
        let (_, fragments_in_line) =
            parse_markdown_text_with_checkbox_policy_until_eol_or_eoi_ng(
                line.content.clone(),
...
#[cfg(test)]
mod tests_parse_block_smart_list_ng {
    use super::*;
    ...
    fn test_with_unicode() {
        ...
        let (remainder, (lines, bullet_kind, indent)) =
            parse_block_smart_list_advance_ng(input).unwrap();
...
fn parse_unordered_list(
    ...
    let (input, content_lines) =
        parse_smart_list_content_lines_ng(input, indent, bullet)?;
...
mod tests_parse_smart_list_ng {
    ...
    fn test_with_unicode_emoji() {
        ...
        let (remainder, actual) = parse_smart_list_ng(input).unwrap();
        assert_eq2!(remainder.is_empty(), true);
        assert_eq2!(
    ...
    fn test_invalid_ul_list() {
        ...
        let actual = parse_smart_list_ng(input);
    ...
    fn test_bullet_kinds() {
        ...
        {
            as_str_slice_test_case!(input, "- foo");
            let (_remainder, actual) = parse_smart_list_ng(input).unwrap();
            assert_eq2!(actual.bullet_kind, BulletKind::Unordered);
        }
    ...
    fn test_indent() {
        ...
        {
            as_str_slice_test_case!(input, " - foo");
            let result = parse_smart_list_ng(input);
...
pub fn parse_smart_list_content_lines_ng<'a>(
    input: AsStrSlice<'a>,
    indent: usize,
    bullet: AsStrSlice<'a>,
) -> IResult<
...
#[cfg(test)]
mod tests_parse_smart_list_content_lines_ng {
    use super::*;
    use crate::{as_str_slice_test_case, assert_eq2};
    ...
    fn test_single_line_no_newline() {
        ...
        let (remainder, lines) =
            parse_smart_list_content_lines_ng(input, indent, bullet).unwrap();
...
mod tests_checkbox_policy {
    ...
    fn test_ignore_checkbox_empty_string() {
        {
            as_str_slice_test_case!(input, "");
            let result = parse_markdown_text_with_checkbox_policy_until_eol_or_eoi_ng(
                input,
                CheckboxParsePolicy::IgnoreCheckbox,
            );
