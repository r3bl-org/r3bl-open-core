...
pub fn from_syntect_to_tui(vec_styled_str: Vec<(SyntectStyle, &str)>) -> List<(Style, UnicodeString)> {
...
impl From<Vec<(SyntectStyle, &str)>> for List<(Style, UnicodeString)> {
  fn from(value: Vec<(SyntectStyle, &str)>) -> Self {
    pub fn from_vec_styled_str(vec_styled_str: &Vec<(SyntectStyle, &str)>) -> List<(Style, UnicodeString)> {
...
impl List<(Style, UnicodeString)> {
  ...
  pub fn clip(&self, scroll_offset_col_index: ChUnit, max_display_col_count: ChUnit) -> StyledTexts {
    // Populated and returned at the end.
    let mut list = List::default();

    // Clip w/out syntax highlighting & store this as a pattern to match against.
    let pattern: &str = &self.get_plain_text_clipped(scroll_offset_col_index, max_display_col_count);
    let mut matcher = PatternMatcherStateMachine::new(pattern, scroll_offset_col_index.into());

    // Main loop over each `text_fragment` in the List (which represents a single line of text).
    for (style, text_fragment) in self.iter() {
      let mut clipped_text_fragment = String::new();

      for segment in text_fragment.iter() {
