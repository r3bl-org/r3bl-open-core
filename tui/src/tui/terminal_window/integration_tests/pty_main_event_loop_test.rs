...
use crate::{App, CONTROLLED_READY, CONTROLLED_STARTING, CommonResult,
            ComponentRegistryMap, EventPropagation, GlobalData, HasFocus, InputDevice,
            InputEvent, Key, KeyPress, OutputDevice, PtyTestContext, RenderPipeline,
            SUCCESS, TerminalWindowMainThreadSignal, generate_pty_test, height,
...
generate_pty_test! {
    ...
    controller: controller_main_event_loop,
    controlled: controlled_main_event_loop,
    mode: crate::PtyTestMode::Cooked,
}

fn controller_main_event_loop(context: PtyTestContext) {
    let PtyTestContext {
        pty_pair,
        child,
        mut buf_reader,
