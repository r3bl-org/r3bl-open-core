...
///
/// This trait converts an [`OffscreenBuffer`] (post-Compositor) into [`RenderOpOutputVec`]
/// (terminal-executable operations), then executes them on the terminal.
///
/// # Type Safety Note
///
/// This trait works with `RenderOpOutputVec` (post-Compositor operations), not `RenderOpIRVec`.
/// The Compositor has already applied all necessary transformations (clipping, Unicode handling, etc.)
