...
use crate::{GetMemSize, InlineVec, PixelChar, TinyInlineString, VPWidth, dim_underline,
...
impl PixelCharLine {
    ...
    #[must_use]
    pub fn new_empty(arg_width: impl Into<VPWidth>, default_val: PixelChar) -> Self {
        let width = arg_width.into().as_usize();
        Self {
            pixel_chars: vec![default_val; width],
        }
    }
}
...
mod tests {
    use super::*;
    use crate::{TuiStyle, vp_width};

    #[test]
    fn test_pixel_char_line_new_empty() {
        let width = vp_width(5);
        let line = PixelCharLine::new_empty(width, PixelChar::Spacer);
    ...
    fn test_pixel_char_line_deref_mut() {
        let width = vp_width(2);
        let mut line = PixelCharLine::new_empty(width, PixelChar::Spacer);
    ...
    fn test_pixel_char_line_equality() {
        let width = vp_width(2);
        let line1 = PixelCharLine::new_empty(width, PixelChar::Spacer);
        let line2 = PixelCharLine::new_empty(width, PixelChar::Spacer);

        assert_eq!(line1, line2);

        // Modify one line.
        let mut line3 = PixelCharLine::new_empty(width, PixelChar::Spacer);
