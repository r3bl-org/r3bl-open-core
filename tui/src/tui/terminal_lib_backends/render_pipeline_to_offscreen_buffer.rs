...
width: {}",
    ...
    // Iterate over ansi text segment*s*.
    for segment in ansi_text_segments.iter() {
        let segment_display_width = segment.display_width;
        if segment_display_width == 0 {
            continue;
        }
        ...
        // Set the `PixelChar` at `insertion_col_index`.
        if line_copy.get(insertion_col_index).is_some() {
            // Since there is no style, no need to have a special case for SPACER.
            line_copy[insertion_col_index] = {
                PixelChar::AnsiText {
            ...
            //    values.
            if segment_display_width > 1 {
                // Deal w/ `segment` that is wider than 1 display width.
                let num_of_extra_display_cols_to_inject_void_into = segment_display_width - 1; // Safe subtract.
                for _ in 0..num_of_extra_display_cols_to_inject_void_into {
                    // Make sure insertion_col_index is safe to access.
                    if line_copy.get(insertion_col_index + 1).is_some() {
                ...
                // `segment` is 1 => move `insertion_col_index` forward by 1.
                // Move insertion_col_index forward.
                insertion_col_index += 1;
            }

            already_inserted_display_width += ch!(segment.display_width);
        } else {
            // Run out of space in the line of the offscreen buffer.
            break;
        }
...
/// Render plain or ANSI text to an offscreen buffer. This will modify the `my_offscreen_buffer`
/// argument.
///
...
mod tests {
    ...
    fn test_ansi_text_render_path() {
        ...
        let text_2 = "world";
        let text_1_color =
            lolcat_each_char_in_unicode_string(&UnicodeString::from(text_1), Some(&mut lolcat));
        lolcat.next_color();
        let text_2_color =
            lolcat_each_char_in_unicode_string(&UnicodeString::from(text_2), Some(&mut lolcat));
        ...
        // Line 1 (row index 0). "╳╳╳╳╳╳😃❯he"
        {
            ...
            assert_eq2!(
                my_offscreen_buffer.buffer[0][6],
                PixelChar::AnsiText {
                    content: "\u{1b}[38;2;132;235;15m😃".into()
                }
            );
            assert_eq2!(my_offscreen_buffer.buffer[0][7], PixelChar::Void);
            assert_eq2!(
                my_offscreen_buffer.buffer[0][8],
                PixelChar::AnsiText {
                    content: "\u{1b}[39m\u{1b}[38;2;136;233;14mh".into()
                }
            );
            assert_eq2!(
                my_offscreen_buffer.buffer[0][9],
                PixelChar::AnsiText {
                    content: "\u{1b}[39m\u{1b}[38;2;140;231;12me".into()
                }
            );
        }

        // Line 2 (row index 1). "╳╳world╳╳╳"
        {
            assert_eq2!(my_offscreen_buffer.buffer[1][0], PixelChar::Spacer);
            assert_eq2!(my_offscreen_buffer.buffer[1][1], PixelChar::Spacer);
            assert_eq2!(
                my_offscreen_buffer.buffer[1][2],
                PixelChar::AnsiText {
                    content: "\u{1b}[38;2;136;233;14mw".into()
                }
            );
            assert_eq2!(
                my_offscreen_buffer.buffer[1][3],
                PixelChar::AnsiText {
                    content: "\u{1b}[39m\u{1b}[38;2;140;231;12mo".into()
                }
            );
            assert_eq2!(
                my_offscreen_buffer.buffer[1][4],
                PixelChar::AnsiText {
                    content: "\u{1b}[39m\u{1b}[38;2;144;228;10mr".into()
                }
            );
            assert_eq2!(
                my_offscreen_buffer.buffer[1][5],
                PixelChar::AnsiText {
                    content: "\u{1b}[39m\u{1b}[38;2;149;225;9ml".into()
                }
            );
            assert_eq2!(
                my_offscreen_buffer.buffer[1][6],
                PixelChar::AnsiText {
                    content: "\u{1b}[39m\u{1b}[38;2;153;223;7md".into()
                }
            );
            assert_eq2!(my_offscreen_buffer.buffer[1][7], PixelChar::Spacer);
            assert_eq2!(my_offscreen_buffer.buffer[1][8], PixelChar::Spacer);
