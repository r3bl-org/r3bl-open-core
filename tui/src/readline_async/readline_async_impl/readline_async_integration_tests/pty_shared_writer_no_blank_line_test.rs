...
//!
//! ```bash
//! cargo test -p r3bl_tui test_pty_shared_writer_no_blank_line -- --nocapture
//! ```
//!
...
use crate::{GLYPH_CONTROLLED, GLYPH_CONTROLLER, GLYPH_SUCCESS, GLYPH_WARNING,
            ...
            OfsBufVT100, PixelChar, PtyTestContext, PtyTestMode, SharedWriter,
            generate_pty_test, ok, readline_async::readline_async_impl::LineState,
            vp_height, vp_width};
use std::io::Write;
...
fn controller(context: PtyTestContext) {
    ...
    eprintln!(
        "{GLYPH_CONTROLLER} PTY Controller: Starting SharedWriter blank line test..."
    );

    let result =
        child.read_until_marker(&mut buf_reader, "CONTROLLED_DONE", |trimmed: &str| {
            // Skip debug lines from the test framework.
            !trimmed.contains(GLYPH_CONTROLLED)
                && !trimmed.contains(MSG_TEST_RUNNING)
                && !trimmed.contains(MSG_CONTROLLED_STARTING)
        });
    ...
    for i in 0..output_lines.len().saturating_sub(1) {
        ...
        if current.is_empty() && (next.starts_with('>') || next.starts_with("$ ")) {
            ...
            eprintln!("  {GLYPH_WARNING} Found blank line at index {i} before prompt!");
        }
    }
    ...
    eprintln!("{GLYPH_SUCCESS} PTY Controller: No blank line detected before prompt!");
}
...
fn controlled() {
    ...
    writeln!(shared_writer, "line 1").expect("conversion error");
    writeln!(shared_writer, "line 2").expect("conversion error");
