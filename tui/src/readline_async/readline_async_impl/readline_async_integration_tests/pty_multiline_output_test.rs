...
//!
//! ```bash
//! cargo test -p r3bl_tui test_pty_multiline_output_starts_at_column_1 -- --nocapture
//! ```
//!
...
fn controller(context: PtyTestContext) {
    ...
    eprintln!(
        "{GLYPH_CONTROLLER} PTY Controller: Starting multi-line output column test..."
    );
    ...
    eprintln!("{GLYPH_SUCCESS} PTY Controller: All lines start at column 1 correctly!");
}
