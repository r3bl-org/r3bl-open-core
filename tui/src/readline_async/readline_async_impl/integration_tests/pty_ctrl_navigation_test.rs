...
fn pty_master_entry_point(
    ...
    // Ctrl+Left is ESC [1;5D
    writer.write_all(b"\x1b[1;5D").expect("Failed to write Ctrl+Left");
    writer.flush().expect("Failed to flush");
    ...
    // Ctrl+Right is ESC [1;5C
    writer.write_all(b"\x1b[1;5C").expect("Failed to write Ctrl+Right");
    writer.flush().expect("Failed to flush");
    std::thread::sleep(Duration::from_millis(100));
