...
use crate::{execute_commands,
            get_key_press_from_input_device,
            CalculateResizeHint,
            ChooseKeyPress,
            FunctionComponent,
            KeyPressReader};
...
pub async fn enter_event_loop_async<S: CalculateResizeHint>(
    ...
    on_keypress: impl Fn(&mut S, ChooseKeyPress) -> EventLoopResult,
    ...
    loop {
        let key_press = get_key_press_from_input_device(input_device).await;
...
pub fn enter_event_loop<S: CalculateResizeHint>(
    state: &mut S,
    function_component: &mut impl FunctionComponent<S>,
    on_keypress: impl Fn(&mut S, ChooseKeyPress) -> EventLoopResult,
