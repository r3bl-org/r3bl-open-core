...
//!     - You can use
//!       [`process::request_shutdown()`] or
//!       [`panic!()`] to kill the entire
//!       process. This is not appealing.
//!     - Even if that task is wrapped in a [`thread::spawn()` or
//!       `thread::spawn_blocking()`], it isn't
...
//! [rustyline-async]. However it has mostly
...
//! [`thread::spawn()` or
//!       `thread::spawn_blocking()`]: https://tokio.rs/tokio/tutorial/spawning
//!
