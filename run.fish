...
function print-help
    ...
    if test "$command" = all
        ...
        echo "    "(set_color green)"log"(set_color normal)"                  Monitor /tmp/r3bl_tui/log.txt"
...
#
# Log sources:
# - /tmp/r3bl_tui/log.txt: Generated by examples and binaries
#
# Usage:
#   fish run.fish log
#
...
function log
    ...
    set log_file "/tmp/r3bl_tui/log.txt"

    if not test -f $log_file
        echo "No log file found at $log_file. Run 'fish run.fish run-examples' or 'fish run.fish run-binaries' with logging enabled first to generate logs."
        return
    end

    echo (set_color magenta)"Monitoring log file: "(set_color green)"$log_file"(set_color normal)
    tail -f -s 5 $log_file
end
