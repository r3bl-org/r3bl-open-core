...
/// a single byte or up to 4 bytes. A "grapheme cluster" can be multiple graphemes or code
/// points or Unicode scalar values.
///
...
///
/// ```rust
/// use r3bl_core::{GCString, GCStringExt as _, ch, col, width, seg_index};
/// let ustr = "📦🙏🏽".grapheme_string();
/// if let Some(segment) = ustr.segments.first() {
...
pub struct Seg {
    ...
    /// the same as the byte size [Self::bytes_size].
    ...
    /// The display width, aka [Self::display_width], may not the same as the byte size.
...
#[cfg(test)]
mod tests {
    use crate::{GCStringExt as _, ch, col, seg_index, width};
