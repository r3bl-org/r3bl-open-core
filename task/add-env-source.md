...
- [ ] Mandatory manual review:
    - [ ] `tui/src/core/script/posix_source/mod.rs`
    - [ ] `tui/src/core/script/posix_source/core.rs`
    - [ ] `tui/src/core/script/posix_source/subshell.rs`
    - [ ] `tui/src/core/script/posix_source/parser.rs`
    - [ ] `tui/src/core/script/posix_source/filter.rs`
    - [ ] `tui/src/core/script/posix_source/diff.rs`
    - [ ] `tui/src/core/script/posix_source/formatters/mod.rs`
    - [ ] `tui/src/core/script/posix_source/formatters/fish.rs`
    - [ ] `tui/src/core/script/posix_source/formatters/json.rs`
    - [ ] `tui/src/core/script/posix_source/formatters/dotenv.rs`
    - [ ] `tui/src/core/script/posix_source/conformance_tests/mod.rs`
    - [ ] `tui/src/core/script/posix_source/conformance_tests/test_fixtures.rs`
    - [ ] `tui/src/core/script/posix_source/conformance_tests/tests/test_subshell_conformance.rs`
    - [ ] `tui/src/core/script/posix_source/conformance_tests/tests/test_golden_formatters.rs`
...
- [x] Verification & Cross-Compilation Checks:
    ...
    - [x] Native Windows build & test execution on `nazmul-win.local` over SSH:
        ...
        - [x] Verify end-to-end `.bat` evaluation
              (`env-source --input-file test.bat --output-format powershell`) inside native
              PowerShell on Windows.
- [x] Restructure and Align `conformance_tests/` for Unix & Windows:
    - [x] Align test directory naming with `md_parser` and `vt_100_pty_output_parser` conventions:
        - [x] Replace `fixtures/` and `golden/` with `conformance_data/` containing `unix/` and `windows/` subdirectories.
        - [x] Use `input_<name>.*` and `expected_<name>.*` naming convention.
        - [x] Create `conformance_data/AGENTS.md` for test data integrity documentation.
        - [x] Create `test_fixtures_env_source.rs` dedicated to test harness helpers, mock initial environment builders (`create_mock_initial_env_unix`, `create_mock_initial_env_windows`), and temp script runners (`run_fixture_sh`, `run_fixture_bat`).
    - [x] Un-gate `test_golden_formatters.rs` to run on all platforms, and add PowerShell golden tests (`test_golden_cargo_env_powershell`, `test_golden_noisy_script_powershell`, `test_golden_edge_cases_all_formats_windows`).
    - [x] Split subshell tests into `test_subshell_unix.rs` (`#[cfg(unix)]`) and `test_subshell_windows.rs` (`#[cfg(windows)]`).
    - [x] Update Windows filter in `filter.rs` to ignore `PROMPT` (the Windows equivalent of Unix `PS1`), and include `PATHEXT` in Windows mock environment.
    - [x] Update `run_shell.rs` on Windows to handle script paths with spaces using `raw_arg`.
- [ ] Mandatory manual review:
    - [ ] `cmdr/Cargo.toml`
    - [ ] `cmdr/src/bin/env-source.rs`
    - [ ] `cmdr/src/env_source/clap_config.rs`
    - [ ] `cmdr/src/env_source/mod.rs`
    - [ ] `tui/src/core/script/env_source/args.rs`
    - [ ] `tui/src/core/script/env_source/core.rs`
    - [ ] `tui/src/core/script/env_source/filter.rs`
    - [ ] `tui/src/core/script/env_source/formatters/mod.rs`
    - [ ] `tui/src/core/script/env_source/formatters/powershell.rs`
    - [ ] `tui/src/core/script/env_source/run_shell.rs`
    - [ ] `tui/src/core/script/env_source/parser.rs`
    - [ ] `tui/src/core/script/env_source/diff.rs`
    - [ ] `tui/src/core/script/env_source/conformance_tests/mod.rs`
    - [ ] `tui/src/core/script/env_source/conformance_tests/conformance_data/mod.rs`
    - [ ] `tui/src/core/script/env_source/conformance_tests/conformance_data/AGENTS.md`
    - [ ] `tui/src/core/script/env_source/conformance_tests/test_fixtures_env_source.rs`
    - [ ] `tui/src/core/script/env_source/conformance_tests/tests/mod.rs`
    - [ ] `tui/src/core/script/env_source/conformance_tests/tests/test_golden_formatters.rs`
    - [ ] `tui/src/core/script/env_source/conformance_tests/tests/test_subshell_unix.rs`
    - [ ] `tui/src/core/script/env_source/conformance_tests/tests/test_subshell_windows.rs`
...
- [x] Update any dotfiles / fresh-install scripts referencing `posix-source` to
      `env-source`.
- [ ] Deploy `env-source` binary and sync configurations across the fleet via
      `env-save -w`.
- [ ] Verify interactive shell startup time across fleet hosts.
- [ ] Mandatory manual review:
    - [ ] `~/scripts/fish/core/06-environment.fish`
    - [ ] Dotfiles references
    - [ ] Fleet sync status logs
