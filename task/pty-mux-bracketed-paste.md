...
## Context
The `PTY` multiplexer currently has a `bracketed_paste` field in `TerminalModeState` (part of the parser state) that is marked with `#[allow(dead_code)]`. The `VT-100` sequences for bracketed paste (`ESC [ ? 2004 h` / `ESC [ ? 2004 l`) are currently ignored by the parser in `vt_100_shim_mode_ops.rs`.
...
- Wire these sequences to update `terminal_mode.bracketed_paste`.
- Remove the `#[allow(dead_code)]` from `bracketed_paste` in `terminal_mode.rs`.
