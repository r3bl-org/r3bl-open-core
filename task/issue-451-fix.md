...
impl<W: RRTWorker> RRT<W> {
    ...
    /// }
    /// ```
    pub fn input_sender(&self) -> InputSender<W> {
        InputSender {
...
pub trait RRTWorker: Send + Debug + 'static {
    type Config: Clone + Send + Sync + 'static;
    type InboundSender: Clone + Send + Sync + 'static;
...
- **How it works**:
  ...
  - The framework stores this `InboundSender` inside the `ThreadState::Running` variant:
    ...
    ```
  - The subscriber queries the current sender using `SubscriberGuard::input_sender()`:
    ```rust
    impl<W: RRTWorker> SubscriberGuard<W> {
        pub fn input_sender(&self) -> Option<W::InboundSender> {
            let state_guard = self.shared_state.lock();
            if let ThreadState::Running(_, ref sender) = *state_guard.state {
                Some(sender.clone())
            } else {
                None
            }
...
- [ ] Update `RRTWorker` trait to define `Config` and `Input` associated types. Rename
      existing `Event` type to `Output`. Rename `CHANNEL_CAPACITY` to
      `OUTPUT_CHANNEL_CAPACITY` and add `INPUT_CHANNEL_CAPACITY`.
- [ ] Update `RRTWorker::create_and_register_os_sources` signature to accept `config` and
      `receiver`, and return the tuple.
- [ ] Update `ThreadState` enum to store the `tokio::sync::broadcast::Sender` in the
      `Running` variant.
- [ ] Add `input_sender_notify: tokio::sync::Notify` to `ThreadLifecycleMonitor` and
      trigger it inside `set_state()`.
- [ ] Implement the `InputSender<W>` struct which wraps `shared_state` and handles
      retry-on-disconnect logic using `input_sender_notify`.
- [ ] Add the `input_sender()` method to `RRT` and `SubscriberGuard`.
- [ ] **Mandatory manual review:** Verify API signatures.
  - [ ] `tui/src/core/resilient_reactor_thread/rrt_worker.rs`
  - [ ] `tui/src/core/resilient_reactor_thread/rrt_thread_state.rs`
  - [ ] `tui/src/core/resilient_reactor_thread/rrt_monitor.rs`
  - [ ] `tui/src/core/resilient_reactor_thread/rrt_input_sender.rs`
  - [ ] `tui/src/core/resilient_reactor_thread/rrt.rs`

### Phase 2: Engine & Spawning Refactoring

- [ ] Update `try_subscribe` function in `rrt.rs` to take `config`, create the initial
      `tokio::sync::broadcast` channel, and store the sender.
- [ ] Update `run_worker_loop` and `perform_restart_retry_loop` in `rrt_engine.rs` to hold
      the config, and recreate the channel upon restart.
- [ ] **Mandatory manual review:** Verify RRT engine loop changes.
  - [ ] `tui/src/core/resilient_reactor_thread/rrt.rs`
  - [ ] `tui/src/core/resilient_reactor_thread/rrt_engine.rs`

### Phase 3: Adapting Existing Workers & Tests

- [ ] Update `MioPollWorker` and other internal RRT workers to use `type Config = ()` and
      `type Input = ()` (or appropriate mock types), and rename `Event` to `Output`.
- [ ] Fix all compile errors in the RRT tests.
- [ ] Run RRT tests to ensure liveness, restart policy, and channel behavior still pass.
- [ ] **Mandatory manual review:** Verify all tests and adapted workers compile and run
      successfully.
  - [ ] `tui/src/core/resilient_reactor_thread/rrt_integration_tests/`
  - [ ] `tui/src/core/resilient_reactor_thread/process_isolated_tests/`
