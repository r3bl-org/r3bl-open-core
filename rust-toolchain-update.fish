...
function clean_and_verify_build
    ...
    if not toolchain_log_command "Running cargo clean..." cargo clean
        toolchain_log "⚠️  cargo clean failed (non-critical)"
    end
...
function validate_toolchain
    ...
    if not set_toolchain_in_toml $toolchain
        toolchain_log "❌ Failed to update rust-toolchain.toml with candidate toolchain"
        return 1
    end
    toolchain_log "✅ rust-toolchain.toml updated with: $toolchain"
    toolchain_log ""
    ...
    for step in $validation_steps
        ...
        if grep -Ei "internal compiler error|thread 'rustc' panicked|error: the compiler unexpectedly panicked|this is a bug in the rust compiler" $temp_output > /dev/null
            toolchain_log "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            toolchain_log "❌ ICE DETECTED in $step_name"
            toolchain_log "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    toolchain_log ""
    toolchain_log "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    toolchain_log "✅ Toolchain $toolchain is STABLE (no ICE detected)"
    toolchain_log "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    toolchain_log ""
...
function find_stable_toolchain
    ...
    for i in (seq 0 $search_window_days)
        ...
        if not rustup toolchain install $candidate_toolchain 2>&1 | tee -a $LOG_FILE
            toolchain_log "❌ Failed to install $candidate_toolchain"
            toolchain_log "Trying next candidate..."
            toolchain_log ""
            continue
        end

        toolchain_log "✅ Successfully installed $candidate_toolchain"
        toolchain_log ""
        ...
        if validate_toolchain $candidate_toolchain
            ...
            toolchain_log "═══════════════════════════════════════════════════════"
            toolchain_log "🎉 FOUND STABLE TOOLCHAIN: $candidate_toolchain"
            toolchain_log "═══════════════════════════════════════════════════════"
            toolchain_log ""
...
function update_toolchain_config
    ...
    if not set_toolchain_in_toml $target_toolchain
        toolchain_log "❌ Failed to update rust-toolchain.toml"
        return 1
    end

    toolchain_log "✅ Successfully updated rust-toolchain.toml"
...
function cleanup_old_toolchains
    ...
    for toolchain in $all_toolchains
        ...
        if rustup toolchain uninstall $toolchain 2>&1 | tee -a $LOG_FILE
            ...
            toolchain_log "    ❌ Failed to remove $toolchain"
        end
    end
...
function main
    ...
    or begin
        toolchain_log "❌ FATAL: Could not find a stable toolchain"
    ...
    toolchain_log "✅ rust-toolchain.toml already updated during validation"

    # Note: No need to install again - find_stable_toolchain already installed it
    toolchain_log "✅ Toolchain $target_toolchain already installed"
    ...
    if rustup update stable 2>&1 | tee -a $LOG_FILE
        toolchain_log "✅ Stable toolchain updated successfully"
    else
        toolchain_log "⚠️  Stable toolchain update had issues (non-critical, continuing)"
    end
    toolchain_log ""
    ...
    # Cleanup
    toolchain_log "═══════════════════════════════════════════════════════"
    toolchain_log "✅ Rust Toolchain Update Completed at "(date)
