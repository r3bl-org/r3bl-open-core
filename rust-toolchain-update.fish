...
function clean_and_verify_build
    ...
    set -l ice_files (find $PROJECT_DIR -name "rustc-ice-*.txt" -type f 2>/dev/null)
    if test (count $ice_files) -gt 0
        log_message "Found "(count $ice_files)" ICE file(s):"
        for ice_file in $ice_files
            log_message "  - $ice_file"
            rm -f $ice_file
        end
        log_message "✅ Removed "(count $ice_files)" ICE file(s)"
    else
        log_message "No ICE files found (this is good!)"
    end
    log_message ""
...
function validate_toolchain
    ...
    set -l validation_steps \
        "clippy:cargo +$toolchain clippy --all-targets" \
        "build:cargo +$toolchain build" \
        "nextest:cargo +$toolchain nextest run" \
        "doctest:cargo +$toolchain test --doc" \
        "doc:cargo +$toolchain doc --no-deps"
        ...
        if validate_toolchain $candidate_toolchain
            ...
            if command -v notify-send >/dev/null 2>&1
                notify-send --urgency=normal \
                    ...
                    "✅ Found stable nightly: $candidate_toolchain\nAttempt "(math $i + 1)"/"(math $search_window_days + 1)" ($days_ago days old)" \
                    2>/dev/null &
                log_message "Success notification sent"
            else
                log_message "notify-send not available, skipping success notification"
            end
...
function main
    ...
    log_message "═══════════════════════════════════════════════════════"
    log_message "Phase 2: Update Configuration and Components"
    log_message "═══════════════════════════════════════════════════════"
    log_message ""

    # Update rust-toolchain.toml with the validated toolchain
    update_toolchain_config
    or return 1

    # Note: No need to install again - find_stable_toolchain already installed it
    log_message "Skipping redundant install - toolchain $target_toolchain already installed"
