...
def main [...args: string] {
    ...
    match $command {
        "build-release-in-docker" => {build-release-in-docker}
        "build" => {build}
        "clean" => {clean}
        "install" => {install}
        "log" => {log}
...
# https://www.nushell.sh/lang-guide/chapters/flow_control/if-else.html#if-else
...
def print-help [command: string] {
    if $command == "all" {
        print $'Usage: (ansi blue_bold)run(ansi reset) (ansi green_bold)<command>(ansi reset) (ansi yellow)[args](ansi reset)'
        print $'(ansi green_bold)<command>(ansi reset) can be:'
        print $'    (ansi green)build-release-in-docker(ansi reset)'
        ...
        print $'Usage: (ansi blue_bold)run(ansi reset) (ansi green_bold)watch-one-test(ansi reset) (ansi yellow)<test-name>(ansi reset)'
        print $'    (ansi green)<test-name>(ansi reset) is the name of the test to watch.'
    } else if $command == "watch-macro-expansion-one-test" {
        print $'Usage: (ansi blue_bold)run(ansi reset) (ansi green_bold)watch-macro-expansion-one-test(ansi reset) (ansi yellow)<test-name>(ansi reset)'
        print $'    (ansi green)<test-name>(ansi reset) is the name of the test to watch.'
    } else {
